...
gettimeofday(&start_time, NULL);
	#pragma omp parallel for ordered(1) private(area, total, dpt, ip, centre, cp, brightness, tmp, x, y, j) use(psbdx, 45)
  for (i=mask_size;i<y_size-mask_size;i++)
  {
	#pragma omp ordered depend(sink: i-1) depend(var: out)
//	#pragma omp parallel for ordered(1) private(area, total, dpt, ip, centre, cp, brightness, tmp, x, y) use(psbdx, 50)
