...
float P7Viterbi(char *dsq, int L, struct plan7_s *hmm, struct dpmatrix_s *mx,
  ...
  for (i = 1; i <= L; i++) {
    ...
    TIME(start_time);
#if defined(USE_TLS)
#pragma omp parallel for ordered(1) private(sc) use(tls, 1)
#endif /* USE_TLS */
#if defined(USE_BDX)
#pragma omp parallel for ordered(1) private(sc) use(psbdx, 1)
#endif /* USE_BDX */
#if defined(ORDERED)
...
#pragma omp ordered depend(source)
    ...
    TIME(end_time);
    double t = timevaldiff(&start_time, &end_time);
		run_time_acc += t;
 ...
 *    AllocPlan7Body()      plan7.c                (data alignment on 16-byte
 boundaries)
 *    CreatePlan7Matrix()   core_algorithms.c      (data alignment on 16-byte
 boundaries)
 *    ResizePlan7Matrix()   core_algorithms.c      (data alignment on 16-byte
 boundaries)
 *    P7Viterbi()           core_algorithms.c      (total recode, w/ Altivec
 instructions)
 ################################################################*/
void AllocPlan7Body(struct plan7_s *hmm, int M) {
  ...
  hmm->tsc[TMI] = (int *)(((((unsigned long int)hmm->tsc_mem) +
                            (M + 12) * sizeof(int) + 15) &
                           (~0xf)) +
                          12);
  hmm->tsc[TMD] = (int *)((
      (((unsigned long int)hmm->tsc_mem) + 2 * (M + 12) * sizeof(int) + 15) &
      (~0xf)));
  hmm->tsc[TIM] = (int *)((
      (((unsigned long int)hmm->tsc_mem) + 3 * (M + 12) * sizeof(int) + 15) &
      (~0xf)));
  hmm->tsc[TII] = (int *)(((((unsigned long int)hmm->tsc_mem) +
                            4 * (M + 12) * sizeof(int) + 15) &
                           (~0xf)) +
                          12);
  hmm->tsc[TDM] = (int *)((
      (((unsigned long int)hmm->tsc_mem) + 5 * (M + 12) * sizeof(int) + 15) &
      (~0xf)));
  hmm->tsc[TDD] = (int *)((
      (((unsigned long int)hmm->tsc_mem) + 6 * (M + 12) * sizeof(int) + 15) &
      (~0xf)));

  for (x = 0; x < MAXCODE; x++) {
    hmm->msc[x] = (int *)(((((unsigned long int)hmm->msc_mem) +
                            x * (M + 1 + 12) * sizeof(int) + 15) &
                           (~0xf)) +
                          12);
    hmm->isc[x] = (int *)(((((unsigned long int)hmm->isc_mem) +
                            x * (M + 12) * sizeof(int) + 15) &
                           (~0xf)) +
                          12);
  }
...
struct dpmatrix_s *CreatePlan7Matrix(int N, int M, int padN, int padM) {
  ...
  for (i = 1; i <= N; i++) {
    mx->xmx[i] =
        mx->xmx[0] +
        i * (5 + 11); /* add 11 bytes per row, making it divisible by 4 */
    n = 12 - (M + 2) % 4;
  ...
  if (padM > 0 && padN > 0)
    Die("there's trouble with RAMLIMIT if you grow in both M and N.");
  mx->maxN = N;
  mx->maxM = M;
  mx->padN = padN;
  mx->padM = padM;
 ...
 */
void ResizePlan7Matrix(struct dpmatrix_s *mx, int N, int M, int ***xmx,
 ...
 * Return:   log P(S|M)/P(S|R), as a bit score
 */
/* This first version of P7Viterbi has been accelerated with Altivec
 * vectorization.
 * On Apple hardware, it is up to a factor 10 faster than the old non-altivec
 * version.
 */
typedef union {
  vector signed int v;
...
float P7Viterbi(char *dsq, int L, struct plan7_s *hmm, struct dpmatrix_s *mx,
  ...
  int *p_tmm, *p_tim, *p_tdm, *p_bsc, *p_msc, *p_tmd, *p_tdd, *p_tmi, *p_tii,
      *p_isc, *p_esc;
  int i, n, k;
  int sc;
  ...
  v1 = vec_splat_s32(-1);
  v2 = vec_splat_s32(0);
  mask1 = (vector unsigned int)vec_sld(v1, v2,
                                       12); /* FF in first pos, rest. are 00 */
  ...
  for (i = 1; i <= L; i++) {
    ...
    /* the 0 element of vectors are aligned 12 bytes up from the 16-byte
     * boundary,
    ...
    for (n = 4, k = 1; k < (hmm->M - 3); n += 32, k += 8) {
      ...
      */
      ...
      /* calc mmx+TMM, imx+TIM, dmx+TDM and XMX+bsc with saturated arithmetic,
       * so
      ...
      /* use a mask to select only the first element from v_save_dmx, rest from
       * v3. */
       ...
       * This one depends on the last position of vector 1, which we just
       * finished.
       */
      v_save_dmx = vec_sld(v3, v3, 12);      /* del */
      ...
      /* Puh! that was the deletion... v3/v_save_dmx now contain the updated dmx
       * data;
    ...
    if (k < (1 + hmm->M)) {
      ...
      */
      ...
      /* calc mmx+TMM, imx+TIM, dmx+TDM and XMX+bsc with saturated arithmetic,
       * so
          * we don't loop if we add the large negative numbers used for
       * -infinity.
          */
      v1 = vec_adds(v1, v5);
      v3 = vec_adds(v3, v7);
