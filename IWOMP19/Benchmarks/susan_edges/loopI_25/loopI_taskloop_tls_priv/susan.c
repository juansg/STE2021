                 ...
                 int max_no, int x_size, int y_size)
{
    ...
    int r_arr[STRIP_SIZE*594];
    //char cond_arr[STRIP_SIZE*594];
    ...
    #pragma omp master
    #pragma omp taskloop grainsize (1) default(none) shared(next_iter_commit,r) firstprivate(in,x_size,bp,max_no,y_size) private(j,p,cp,n,i,cont,r_arr)   //spec(n)

    for (is=3; is<y_size-3; is+=STRIP_SIZE)
    {

        char execute_in_htm;
        unsigned status;

        //int r_arr[STRIP_SIZE*(x_size-6)];
        char cond_arr[STRIP_SIZE*(x_size-6)];


        //char cond_arr[STRIP_SIZE*600]={0};
...
Retry:
        ...
        for (i=is; i-is< STRIP_SIZE && i<y_size-3; i++)
        {
