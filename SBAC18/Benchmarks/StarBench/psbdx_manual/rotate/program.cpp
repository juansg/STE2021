...
void* BDX_Thread_Source(void* _param) {
	RotateEngine* bdx_buffer1[BDX_BATCH_SIZE];
	int MyId = *(int *) _param;
...
int main(int argc, char* argv[]) {
    ...
    while (1) {
        ...
        //printf("end) thread 0 stage 1\n");
        bdx_stage_flag[0].value = (bdx_stage_flag[0].value + 1) % bdx_NumThreads;
        /** End of Stage 1 **/


        /** Now for PS-BDX I joined Stage 2 and 3 **/
        /** Begin Stage 2 and 3 **/
        //printf("begin) thread 1 stage 2\n");
	...
	/* Wait confirmation that the other threads have finished. */
