  ...
  for (i = 3; i < y_size - 3; i++)
    for (j = 3; j < x_size - 3; j++) {
      ...
      if (n <= max_no)
		...
		global_mask_size = 4; // hardcoded as in the loop bellow

  gettimeofday(&start_time, NULL);
#if defined(USE_TLS)
#pragma omp parallel for ordered(1) use(tls,1) \
	firstprivate(mid,r,max_no,bp,x_size,in) \
	private(j,w,y,x,b,a,m,do_symmetry,z,n,c,cp,p) num_threads(4)
#endif /* USE_TLS */
#if defined(USE_BDX)
#pragma omp parallel for ordered(1) private(                                   \
    m, n, cp, p, x, y, c, z, do_symmetry, w, a, b, j) use(psbdx, 34)
#endif /* USE_BDX */
#if defined(ORDERED)
#pragma omp parallel for ordered(1) private(m, n, cp, p, x, y, c, z, do_symmetry, w, a, b, j)
#endif /* ORDERED */
#if defined(PARALLEL)
#pragma omp parallel for private(m, n, cp, p, x, y, c, z, do_symmetry, w, a, b, j)
#endif /* PARALLEL */
  for (i = 4; i < y_size - 4; i++) {
