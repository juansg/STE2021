...
uchar c,*p,*cp;
  ...
  next_iter_commit=5;
  //next_itr_commit=&next_iter_commit;
  ...
  #pragma omp master
  #pragma omp taskloop grainsize(1) default(none) shared(next_iter_commit,n) firstprivate(x_size,y_size,r,corner_list,cgx,cgy,in) private(j,x,i)   //spec(n)
  for (is=5;is<y_size-5;is+=STRIP_SIZE){
    ...
    for (i=is; i-is< STRIP_SIZE && i<y_size-5; i++){
