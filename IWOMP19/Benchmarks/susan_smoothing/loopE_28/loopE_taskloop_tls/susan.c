                     ...
                     int x_size, int y_size, uchar* bp)
{
    ...
    if (three_by_three==0)
    {
        ...
        for (i=mask_size; i<y_size-mask_size; i++)
        {
            ...
            #pragma omp taskloop use(tls,45) default(none) firstprivate(dp,in,mask_size,x_size,bp,increment,i,out) private(area,total,dpt,j,tmp,brightness,x,y,cp,centre,ip) 
