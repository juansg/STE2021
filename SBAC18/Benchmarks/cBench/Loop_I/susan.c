...
void susan_edges(uchar *in, int *r, uchar *mid, uchar *bp, int max_no,
  ...
  memset(r, 0, x_size * y_size * sizeof(int));
	...
	global_mask_size = 3; // hardcoded as in the loop bellow

  gettimeofday(&start_time, NULL);
#if defined(USE_TLS)
#pragma omp parallel for ordered(1) use(tls,2) \
	firstprivate(in,x_size,bp,max_no,r) private(j,p,cp,n) num_threads(4)
#endif /* USE_TLS */
#if defined(USE_BDX)
#pragma omp parallel for ordered(1) private(n, p, cp, j) use(psbdx, 37)
#endif /* USE_BDX */
#if defined(ORDERED)
#pragma omp parallel for ordered(1) private(n, p, cp, j)
#endif /* ORDERED */
#if defined(PARALLEL)
#pragma omp parallel for private(n, p, cp, j)
#endif /* PARALLEL */
  for (i = 3; i < y_size - 3; i++) {
