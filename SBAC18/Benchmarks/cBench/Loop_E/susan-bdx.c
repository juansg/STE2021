...
/* }}} */
	...
	fprintf(f, "%d - %d\n", y_size, mask_size);
	fclose(f);
//	#pragma omp parallel for ordered(1) private(area, total, dpt, ip, centre, cp, brightness, tmp, x, y, j) use(psbdx)
  for (i=mask_size;i<y_size-mask_size;i++)
  {
gettimeofday(&start_time, NULL);
	#pragma omp parallel for ordered(1) private(area, total, dpt, ip, centre, cp, brightness, tmp, x, y) use(psbdx, 60)
