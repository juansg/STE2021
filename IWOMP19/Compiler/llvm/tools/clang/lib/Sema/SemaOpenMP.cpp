...
StmtResult Sema::ActOnOpenMPForDirective(
  ...
  if (!AStmt)
	...
	OMPUseClause *useClause = nullptr;
	for(auto C = Clauses.begin(); C != Clauses.end(); C++) {
		if(isa<OMPOrderedClause>(*C)) {
      const OMPOrderedClause* orderedClause =
        static_cast<const OMPOrderedClause*>(*C);
      Expr* numForLoopsExpr = orderedClause->getNumForLoops();
      int num_loops = 1;
      if (numForLoopsExpr != nullptr) {
        ExprResult NumForLoopsResult =
          VerifyPositiveIntegerConstantInClause(numForLoopsExpr, OMPC_ordered,
              /*isStrictlyPositive*/ true);
        if ( NumForLoopsResult.isInvalid() ) {
          return StmtError();
        }
        llvm::APSInt Result;
        VerifyIntegerConstantExpression(numForLoopsExpr, &Result);
        num_loops = Result.getZExtValue();
      }
			while(num_loops > 0) {
        num_loops--;
        CapturedStmt* CAStmt = static_cast<CapturedStmt*>(AStmt);
        const CapturedDecl* CD = CAStmt->getCapturedDecl();
        ForStmt *forStmt = static_cast<ForStmt*>(CD->getBody());
        Stmt *inner = forStmt->getBody();
        if (inner == nullptr) {
          Diag(numForLoopsExpr->getExprLoc(),
                       diag::err_omp_wrong_ordered_loop_count)
            << numForLoopsExpr->getSourceRange();
          return StmtError();
        }
        if(isa<CompoundStmt>(inner)) {
          const CompoundStmt* CS = static_cast<CompoundStmt*>(inner);
          inner = *(CS->body_begin());
        }
        CAStmt->getCapturedDecl()->setBody(inner);
			}
		}
		else if(isa<OMPUseClause>(*C)) {
			useClause = static_cast<OMPUseClause*>(*C);
      if (useClause->getUseKind() != OMPC_USE_bdx &&
          useClause->getUseKind() != OMPC_USE_psbdx  &&
          useClause->getUseKind() != OMPC_USE_tls) {
        useClause = nullptr;
			}
		}
	}
	...
	if (useClause) {
		IdentifierTable *idt = new IdentifierTable((*this).getLangOpts());
    CapturedStmt* CS = static_cast<CapturedStmt*>(AStmt);
    ForStmt* forStmt = static_cast<ForStmt*>(CS->getCapturedStmt());
		Stmt *forInit = forStmt->getInit();
		BinaryOperator *boInit = static_cast<BinaryOperator*>(forInit);
    DeclRefExpr* varDeclRefExpr = static_cast<DeclRefExpr*>(boInit->getLHS());
		VarDecl *forCondVar = static_cast<VarDecl*>(varDeclRefExpr->getDecl());
		...
		DeclarationNameInfo *dniforCondVar =
      new DeclarationNameInfo(*dnforCondVar, SourceLocation());

    Expr *dreforCondVar = DeclRefExpr::Create(getASTContext(),
        NestedNameSpecifierLoc(), SourceLocation(), forCondVar, false,
        *dniforCondVar, forCondVar->getType(),
        clang::ExprValueKind::VK_RValue);
    Expr *dreforCondVar2 = DeclRefExpr::Create(getASTContext(),
        NestedNameSpecifierLoc(), SourceLocation(), forCondVar, false,
        *dniforCondVar, forCondVar->getType(),
        clang::ExprValueKind::VK_RValue);
		
    BinaryOperator *boIncAdd;
		if (useClause->getChunkSize()) {
			Expr *ce = useClause->getChunkSize();
			if (ce->getType() != forCondVar->getType()) {
        ce = ImplicitCastExpr::Create(getASTContext(), forCondVar->getType(),
            CastKind::CK_IntegralCast, ce, nullptr, ExprValueKind::VK_RValue);
      }
      boIncAdd = new (getASTContext()) BinaryOperator(dreforCondVar, ce,
          BinaryOperatorKind::BO_Add, forCondVar->getType(),
          clang::ExprValueKind::VK_RValue, ExprObjectKind::OK_Ordinary,
          SourceLocation(), false);
		}
		else {
      boIncAdd = new (getASTContext()) BinaryOperator(dreforCondVar,
          IntegerLiteral::Create(getASTContext(),
            llvm::APInt(getASTContext().getTypeSize(forCondVar->getType()),
              (uint64_t) 1), forCondVar->getType(), SourceLocation()),
          BinaryOperatorKind::BO_Add, forCondVar->getType(),
          clang::ExprValueKind::VK_RValue, ExprObjectKind::OK_Ordinary,
          SourceLocation(), false);
		}
    BinaryOperator *boIncAdd2 = new (getASTContext())
      BinaryOperator(dreforCondVar2, boIncAdd, BinaryOperatorKind::BO_Assign,
          forCondVar->getType(), clang::ExprValueKind::VK_RValue,
          ExprObjectKind::OK_Ordinary, SourceLocation(), false);
    forStmt->setInc(boIncAdd2);
	}
...
StmtResult Sema::ActOnOpenMPParallelForDirective(
  ...
  getCurFunction()->setHasBranchProtectedScope();
  return OMPParallelForDirective::Create(Context,
      StartLoc, EndLoc, NestedLoopCount, Clauses, AStmt, B,
      DSAStack->isCancelRegion());
}
  ...
  for (auto *C : Clauses) {
    if (auto *DC = dyn_cast<OMPDependClause>(C)) {
      ...
      if (DC->getDependencyKind() == OMPC_DEPEND_source) {
		...
		else if (DC->getDependencyKind() == OMPC_DEPEND_var) {
        llvm::errs() << "Depend VAR!\n";
      }
...
OMPClause *Sema::ActOnOpenMPSingleExprClause(OpenMPClauseKind Kind, Expr *Expr,
  ...
  case OMPC_if:
  case OMPC_default:
// Luis Felipe - Case for Use clause
  case OMPC_use:
// OMP check clause
  case OMPC_check:
  case OMPC_proc_bind:
...
OMPClause *Sema::ActOnOpenMPSimpleClause(
  ...
  case OMPC_default:
    ...
    break;
// Luis Felipe - Redirect to the Use clause handler
  case OMPC_use:
    /*Res =
        ActOnOpenMPUseClause(static_cast<OpenMPUseClauseKind>(Argument),
                                 ArgumentLoc, StartLoc, LParenLoc, EndLoc, nullptr);*/
    break;
  case OMPC_proc_bind:
...
// Luis Felipe - Semantic handler for Use clause
OMPClause *Sema::ActOnOpenMPUseClause(OpenMPUseClauseKind Kind,
                                          SourceLocation KindKwLoc,
                                          SourceLocation StartLoc,
                                          SourceLocation LParenLoc,
                                          SourceLocation EndLoc, Expr *ChunkSize) {
  if (Kind == OMPC_USE_unknown) {
    static_assert(OMPC_USE_unknown > 0,
                  "OMPC_USE_unknown not greater than 0");
    Diag(KindKwLoc, diag::err_omp_unexpected_clause_value)
        << getListOfPossibleValues(OMPC_use, /*First=*/0,
                                   /*Last=*/OMPC_USE_unknown)
        << getOpenMPClauseName(OMPC_use);
    return nullptr;
  }
  ...
  return new (Context)
      OMPUseClause(Kind, KindKwLoc, StartLoc, LParenLoc, EndLoc, ValExpr);
}
...
OMPClause *Sema::ActOnOpenMPSingleExprWithArgClause(
  ...
  case OMPC_collapse:
  case OMPC_default:
// Luis Felipe - Case for Doacross clause
  case OMPC_use:
    Res =
        ActOnOpenMPUseClause(static_cast<OpenMPUseClauseKind>(Argument[0]),
                                 ArgumentLoc[0], StartLoc, LParenLoc, EndLoc, Expr);
    break;
// OMP check clause
  case OMPC_check:
  case OMPC_proc_bind:
...
Sema::ActOnOpenMPDependClause(OpenMPDependClauseKind DepKind,
  ...
  if (DSAStack->getCurrentDirective() == OMPD_ordered &&
      DepKind != OMPC_DEPEND_source && DepKind != OMPC_DEPEND_sink && DepKind != OMPC_DEPEND_var) {
  ...
  if ((DepKind != OMPC_DEPEND_sink && DepKind != OMPC_DEPEND_source && DepKind != OMPC_DEPEND_var) ||
    ...
    for (auto &RefExpr : VarList) {
      ...
      if (DepKind == OMPC_DEPEND_sink) {
		...
		else if (DepKind == OMPC_DEPEND_var) {
 			llvm::errs() << "Handle VAR!\n";
	  } else {
    ...
    if (DepKind != OMPC_DEPEND_source && DepKind != OMPC_DEPEND_sink && DepKind != OMPC_DEPEND_var &&
        Vars.empty())
      return nullptr;
  }
