...
static kmp_int32 __kmp_push_task(kmp_int32 gtid, kmp_task_t *task) {
  ...
  if (TCR_4(thread_data->td.td_deque_ntasks) >=
    ...
    if ((!__kmp_speculative_taskloop) && __kmp_enable_task_throttling &&
...
static kmp_task_t *__kmp_remove_my_task(kmp_info_t *thread, kmp_int32 gtid,
  ...
  if (__kmp_speculative_taskloop){
	  taskdata = thread_data -> td.td_deque[ thread_data -> td.td_deque_head ];
		 // Bump head pointer and Wrap.
	  thread_data -> td.td_deque_head = ( thread_data -> td.td_deque_head + 1 ) & TASK_DEQUE_MASK(thread_data->td);
  }
  else{
  ...
  if (!__kmp_speculative_taskloop) thread_data->td.td_deque_tail = tail;
