...
NEXT:
    ...
    arc = arcs + group_pos;
	int n = 0;
    for( ; arc < stop_arcs; arc += nr_group ) n++;
	//#pragma omp parallel for ordered(1) private(arc, red_cost, basket_size_priv) num_threads(3)// use(psbdx)
	for(int i = 0; i < n; i++)
    {
	    arc = arcs + group_pos;
//		for(int j = 0; j < i; j++) arc += nr_group;
		for(int j = 0 ; arc < stop_arcs; arc += nr_group ) j++;
//		arc = nr_group*i + arcs + group_pos;
		long temp;
		int cond1 = 0, cond2 = 0;
		...
		//#pragma omp ordered depend(sink: i-1) //depend(var: basket_size)
		if(cond1 == 1 && cond2 == 1) {
			basket_size++;
			basket_size_priv = basket_size+1;	
		}
        ...
        if(cond1 == 1)
        {
            ...
            if(cond1 == 1 && cond2 == 1)
            {
                perm[basket_size_priv]->a = arc;
                perm[basket_size_priv]->cost = red_cost;
                perm[basket_size_priv]->abs_cost = ABS(red_cost);
