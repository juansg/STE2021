...
#pragma omp parallel for ordered(1) use(tls,1) \
	firstprivate(dp,in,mask_size,x_size,bp,increment,in_aux) \
	private(area,total,dpt,j,tmp,brightness,x,y,cp,centre,ip,out) num_threads(4)
#endif /* USE_TLS */
#if defined(USE_BDX)
#pragma omp parallel for ordered(1) private(                                   \
    area, total, dpt, ip, centre, cp, brightness, tmp, x, y, j) use(psbdx, 45)
#endif /* USE_BDX */
#if defined(ORDERED)
#pragma omp parallel for ordered(1) private(                                   \
    area, total, dpt, ip, centre, cp, brightness, tmp, x, y, j)
#endif /* ORDERED */
#if defined(PARALLEL)
...
#if defined(USE_BDX)
#pragma omp ordered depend(sink : i - 1) depend(var : out)
#endif /* USE_BDX */
#if defined(ORDERED)
...
#if defined(USE_TLS)
			out = in_aux + (i - mask_size) * (x_size - mask_size - mask_size);
#endif /* USE_TLS */
