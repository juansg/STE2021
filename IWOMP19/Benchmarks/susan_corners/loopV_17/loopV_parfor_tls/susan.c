...
uchar c,*p,*cp;
  ...
  #pragma omp parallel for ordered(1) use(tls, 9) default(none) firstprivate(x_size,y_size,r,corner_list,cgx,cgy,in) shared(n) private(j,x) num_threads(4)   //spec(n)
  for (i=5;i<y_size-5;i++){
    for (j=5;j<x_size-5;j++) {
       x = r[i*x_size+j];
       if (x>0)  {
