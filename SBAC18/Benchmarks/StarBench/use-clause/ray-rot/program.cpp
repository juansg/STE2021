...
bool parseArgs(string *args, unsigned int &angle, unsigned int &xres,
               unsigned int &yres, unsigned int &rpp, string &inname,
               string &outname);
...
#pragma omp parallel for ordered(1) use(tls,6) \
	firstprivate(srcfiles,xres,yres,rpp,angle,destfiles) num_threads(4)
#endif /* __USE_TLS__ */
