                     ...
                     int x_size, int y_size, uchar* bp)
{
    ...
    if (three_by_three==0)
    {
        ...
        for (i=mask_size; i<y_size-mask_size; i++)
        {
            ...
            #pragma omp parallel for schedule(static,1) default(none) shared(next_iter_commit) firstprivate(dp,in,mask_size,x_size,bp,increment,i,out) private(area,total,dpt,j,tmp,brightness,x,y,cp,centre,ip,out_arr)  num_threads(4) //reduction(+:n) //linear(seed:13)  //spec(n) lastprivate(out)
