...
/*
*   This program is a benchmark kernel written as part of the StarBench
* benchmark suite.
*   It is a fully parallelized RGB to YUV color converter, processing .ppm
* format pictures.
*   This application is a common one for embedded domain multiprocessors such as
* DSPs.
*/
...
void statsme();
double timevaldiff(timer *starttime, timer *finishtime);
...
double timevaldiff(timer *starttime, timer *finishtime) {
  double msec;
  msec = (finishtime->tv_sec - starttime->tv_sec) * 1.0e3;
  msec += (finishtime->tv_usec - starttime->tv_usec) / 1.0e3;
  return msec;
}
...
*   ----------------------
*   Called to perform the actual image conversion and disk output. Controls
* iterations.
*/
int processImage(rgbyuv_args_t *args) {
...
*   -------------------------
*   Receives the buffer containing the converted image and separately writes Y,
* U and V component
...
int main(int argc, char **argv) {
  ...
  // Who we are
  fprintf(stdout, "StarBench - RGBYUV Kernel\n");
...
#pragma omp parallel for ordered(1) use(tls,2) \
	firstprivate(srcfiles) num_threads(4)
#endif /* __USE_TLS__ */
