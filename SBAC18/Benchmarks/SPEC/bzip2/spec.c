...
int main (int argc, char *argv[]) {
	...
	if (strstr(input_name, "liberty") - input_name > 0)
		MaxIterations=6000;
	else if (strstr(input_name, "text") - input_name > 0)
		MaxIterations=58000;
	else if (strstr(input_name, "source") - input_name > 0)
		MaxIterations=58000;
	else {
		printf("Cesar) Incorrect parameter. Please check sources at %s:%d\n", __FILE__, __LINE__);
	}
	MaxIterations=6000;	
...
//#endif
	...
	for (i = 0; i*VALIDATE_SKIP < input_size*MB; i++) {
		...
		//cesar: Removed this because we changed the number of 
		//iterations in the parallel loop
