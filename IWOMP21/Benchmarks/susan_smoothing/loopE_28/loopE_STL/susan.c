...
/* }}} */
  ...
  { 
     ...
     next_iter_commit=mask_size;

     gettimeofday(&start_time, NULL);
  

  #pragma omp taskloop grainsize(1) default(none) shared(next_iter_commit,out) firstprivate(dp,in,mask_size,x_size,bp,increment,i) private(area,total,dpt,j,tmp,brightness,x,y,cp,centre,ip)   //reduction(+:n) //linear(seed:13)  //spec(n) lastprivate(out)
    for (js = mask_size; js < x_size-mask_size; js+=STRIP_SIZE){
        char execute_in_htm;
        unsigned status;
                uchar out_arr [STRIP_SIZE];

	///*
	     while (1){ //printf("trying iteration: %d T%d\n", js,omp_get_thread_num());
	...
	*/     
	...
	for (j=js; ((j-js)< STRIP_SIZE) && (j<(x_size-mask_size)); j++){
		...
		*/		
		...
		if (tmp==0)
			out_arr[j-js]=median(in,i,j,x_size);
		else
			out_arr[j-js]=((total-(centre*10000))/tmp);
       ...
       if (execute_in_htm)
       {
        ...
        for (j=js; ((j-js)< STRIP_SIZE) && (j<(x_size-mask_size)); j++){	 
		  //out=out_aux+j-mask_size;     
                out[(i-mask_size)*(x_size-2*mask_size)+j-mask_size]=out_arr[j-js];
  ...
  //out_aux=out_aux+(x_size-mask_size-mask_size);
  //out_aux=out;
}
