...
int main1(int argc, char *argv[], int print)
{
    ...
    for (i = 0; i < FUNCS; i++)
    {
        ...
        */
        ...
        n=0;
        //seed=1;
        #pragma omp taskloop grainsize(1) firstprivate(iterations,i,pBitCntFunc) private(seed,j,nL) shared(next_iter_commit,n) default(none) //reduction(+:n) //linear(seed:13)  //spec(n)
        for (js = 0; js < iterations; js+=STRIP_SIZE)
        {
            //printf("iteracion %ld, thread %d\n", js, omp_get_thread_num());
            char execute_in_htm;
            unsigned status;
