...
//long long loop_carried=0;
//long long invocations=0;


//double tiempo_promedio=0;

//int next_iter_commit __attribute__((aligned(64)));
...
{
float z;
int   do_symmetry, i, j, m, n, a, b, x, y, w,cont;
int is;
...
uchar c,*p,*cp;
  ...
  #pragma omp taskloop grainsize(1) default(none) shared(next_iter_commit,mid) firstprivate(r,max_no,bp,x_size,in,y_size) private(cont,i,j,w,y,x,b,a,m,do_symmetry,z,n,c,cp,p,mid_arr) //untied   //spec(n)
  
  for (is=4;is<y_size-4;is+=STRIP_SIZE){

    char execute_in_htm;
    unsigned status;
    //uchar mid_arr[STRIP_SIZE*592];      

    char cond_arr[STRIP_SIZE*592]={0};
    
    cont=-1;

    Retry:
    //printf("trying %d ,  next %d, thread %d, %d\n", is, next_iter_commit, omp_get_thread_num(),next_iter_commit-is);
    if ((next_iter_commit)!=is)
    {
                execute_in_htm=1;
                status = _xbegin();
                if (status!=_XBEGIN_STARTED){
                   //printf("aborté T%d iter%d next%d\n",omp_get_thread_num(),is,next_iter_commit);
                   goto Retry;
    ...
    for (i=is; i-is< STRIP_SIZE && i<y_size-4; i++){
        ...
        for (i=is; i-is< STRIP_SIZE && i<y_size-4; i++){
    	     ...
    	     for (j=4;j<x_size-4;j++){
		      cont++;
		      if (cond_arr[cont]==1) mid[i*x_size+j]=mid_arr[cont];
		     }
   	     }
...
/* }}} */
  ...
  //printf("Invocations %lld, Iterations %lld, Loop-carried %lld, lc %f\n",invocations,iterations,loop_carried,loop_carried*1.0/iterations);
}
