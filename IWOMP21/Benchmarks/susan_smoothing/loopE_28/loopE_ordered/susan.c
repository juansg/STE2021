...
/* }}} */
  ...
  #pragma omp parallel for ordered(1) default(none) schedule(auto) shared(out) firstprivate(dp,in,mask_size,x_size,bp,increment,i) private(area,total,dpt,j,tmp,brightness,x,y,cp,centre,ip)  num_threads(4) // linear(out:1) lastprivate(out)   
