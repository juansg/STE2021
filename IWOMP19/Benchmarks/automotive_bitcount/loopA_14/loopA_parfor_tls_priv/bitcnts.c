...
int main1(int argc, char *argv[], int print)
{
    ...
    for (i = 0; i < FUNCS; i++)
    {
        ...
        */
        ...
        n=0;
        //seed=1;
        #pragma omp parallel for schedule(static,1) firstprivate(iterations,i,pBitCntFunc) private(seed,j,nL) shared(next_iter_commit,n) num_threads(4) default(none) //reduction(+:n) //linear(seed:13)  //spec(n)
