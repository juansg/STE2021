...
*/
    ...
    n=0;
    //seed=1;
    #pragma omp parallel for ordered(1) default(none) schedule(auto) firstprivate(i,pBitCntFunc,iterations) private(seed,temp) shared(n)  num_threads(4) //shared(iterations) //reduction(+:n) //linear(seed:13)  //spec(n)
