...
void usage(void) {
  ...
  printf("-d <thresh> : Distance threshold, smoothing mode, (default=4) (use "
         "next option instead for flat 3x3 mask)\n");
  printf("-3 : Use flat 3x3 mask, edges or smoothing mode\n");
  printf("-n : No post-processing on the binary edge map (runs much faster); "
         "edges mode\n");
  printf("-q : Use faster (and usually stabler) corner mode; edge-like corner "
         "suppression not carried out; corners mode\n");
  printf("-b : Mark corners/edges with single black points instead of black "
         "with white border; corners or edges mode\n");
  printf("-p : Output initial enhancement image only; corners or edges mode "
...
void get_image(char filename[200], unsigned char **in, int *x_size,
               int *y_size) {
  FILE *fd;
  char header[100];
  int tmp;
...
void susan_principle(uchar *in, int *r, uchar *bp, int max_no, int x_size,
                     int y_size) {
  int i, j, n;
  uchar *p, *cp;
...
void susan_principle_small(uchar *in, int *r, uchar *bp, int max_no, int x_size,
                           int y_size) {
  int i, j, n;
  uchar *p, *cp;
...
void enlarge(uchar **in, uchar *tmp_image, int *x_size, int *y_size,
             int border) {
  int i, j;
...
#if defined(USE_BDX)
__attribute__((noinline)) int __bdx_cond__(int flag) {
  asm("");
  return 1;
}
__attribute__((noinline)) void __bdx_stage_begin__(volatile int flag) {
  asm("");
}
...
#if defined(USE_TLS)
#include <immintrin.h>
void _xabort2(char n __attribute__((unused))) { _xabort(0xff); }
#endif /* USE_TLS */

extern double run_time_acc;
extern int global_x_size, global_y_size, global_mask_size;
...
void susan_smoothing(int three_by_three, uchar *in, float dt, int x_size,
  ...
  if (three_by_three == 0) { /* large Gaussian masks */
    ...
    // Publish x_size, y_size and mask_size
    global_x_size = x_size;
    global_y_size = y_size;
    global_mask_size = mask_size;

    for (i = mask_size; i < y_size - mask_size; i++) {
      gettimeofday(&start_time, NULL);
#if defined(USE_TLS)
#pragma omp parallel for ordered(1) use(tls, 15) \
	firstprivate(dp, in, mask_size, x_size, bp, increment, i, out_aux) \
	private(area, total, dpt, ip, centre, cp, brightness, tmp, x, y) \
	lastprivate(out) num_threads(4)
#endif /* USE_TLS */
#if defined(USE_BDX)
#pragma omp parallel for ordered(1) private(                                   \
    area, total, dpt, ip, centre, cp, brightness, tmp, x, y) use(psbdx, 60)
#endif /* USE_BDX */
#if defined(ORDERED)
#pragma omp parallel for ordered(1) private(area, total, dpt, ip, centre, cp,  \
                                            brightness, tmp, x, y)
#endif /* ORDERED */
#if defined(PARALLEL)
#pragma omp parallel for private(area, total, dpt, ip, centre, cp, brightness, \
                                 tmp, x, y)
#endif /* PARALLEL */
...
#if defined(USE_TLS)
			out = out_aux + j - mask_size;
#endif /* USE_TLS */

#if defined(USE_BDX)
#pragma omp ordered depend(sink : j - 1) depend(var : out)
#endif /* USE_BDX */
#if defined(ORDERED)
...
void edge_draw(uchar *in, uchar *mid, int x_size, int y_size,
               int drawing_mode) {
  int i;
  uchar *inp, *midp;
...
/* {{{ right angle ends - not currently used */
            ...
            if ((b00 & b01) | (b00 & b10) | (b02 & b01) | (b02 & b12) |
                (b20 & b10) | (b20 & b21) | (b22 & b21) |
                (b22 & b12)) { /* case; right angle ends. clean up.
...
void susan_edges(uchar *in, int *r, uchar *mid, uchar *bp, int max_no,
...
void susan_edges_small(uchar *in, int *r, uchar *mid, uchar *bp, int max_no,
                       int x_size, int y_size) {
  float z;
  int do_symmetry, i, j, m, n, a, b, x, y, w;
  uchar c, *p, *cp;
...
void corner_draw(uchar *in, CORNER_LIST corner_list, int x_size,
                 int drawing_mode) {
  uchar *p;
  int n = 0;
...
void susan_corners(uchar *in, int *r, uchar *bp, int max_no,
...
void susan_corners_quick(uchar *in, int *r, uchar *bp, int max_no,
                         CORNER_LIST corner_list, int x_size, int y_size) {
  int n, x, y, i, j;
  uchar *p, *cp;
...
void main1(int argc, char *argv[]) {
  ...
  int *r, argindex = 3, bt = 20, principle = 0, thin_post_proc = 1,
          ...
          max_no_corners = 1850, max_no_edges = 2650, mode = 0, x_size, y_size;
