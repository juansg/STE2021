...
/* }}} */
	...
	fprintf(f, "%d - %d\n", x_size, mask_size);
	fclose(f);
//	#pragma omp parallel for ordered(2) num_threads(2) private(area, total, dpt, ip, centre, cp, brightness, tmp, x, y)// use(psbdx)
  for (i=mask_size;i<y_size-mask_size;i++)
  {
gettimeofday(&start_time, NULL);
	#pragma omp parallel for ordered(1) private(area, total, dpt, ip, centre, cp, brightness, tmp, x, y)
