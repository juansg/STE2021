...
/* }}} */
  ...
  #pragma omp parallel for ordered(1) use(tls,45) default(none) firstprivate(dp,in,mask_size,x_size,bp,increment,i,out) private(area,total,dpt,j,tmp,brightness,x,y,cp,centre,ip)  num_threads(4) // linear(out:1) lastprivate(out)   
        ...
        for(x=-mask_size; x<=mask_size; x++)
	{
      ...
      if (tmp==0)
        out[(i-mask_size)*(x_size-2*mask_size)+j-mask_size]=median(in,i,j,x_size);
      else
        out[(i-mask_size)*(x_size-2*mask_size)+j-mask_size]=((total-(centre*10000))/tmp);
    }
