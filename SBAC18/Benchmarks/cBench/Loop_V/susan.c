...
void susan_corners(uchar *in, int *r, uchar *bp, int max_no,
  ...
  global_mask_size = 5; // hardcoded as in the loop bellow
 
	int flag;
  gettimeofday(&start_time, NULL);
#if defined(USE_TLS)
#pragma omp parallel for ordered(1) use(tls, 1) \
	firstprivate(x_size, r, corner_list, cgx, cgy, in) \
	private(j, x, nL) num_threads(4)
#endif /* USE_TLS */
#if defined(USE_BDX)
#pragma omp parallel for ordered(1) private(x, flag, j) use(psbdx, 40)
#endif /* USE_BDX */
#if defined(ORDERED)
#pragma omp parallel for ordered(1) private(x, flag, j)
#endif /* ORDERED */
#if defined(PARALLEL)
#pragma omp parallel for private(x, flag, j)
#endif /* PARALLEL */
  for (i = 5; i < y_size - 5; i++) {
#if defined(USE_BDX)
#pragma omp ordered depend(sink : i - 1) depend(var : n)
#endif /* USE_BDX */
#if defined(ORDERED)
...
#if defined(USE_TLS)
			nL=n;
			corner_list[nL].info=0;
			corner_list[nL].x=j;
			corner_list[nL].y=i;
			corner_list[nL].dx=cgx[i*x_size+j];
			corner_list[nL].dy=cgy[i*x_size+j];
			corner_list[nL].I=in[i*x_size+j];
			nL++;
			n=nL;
#endif /* USE_TLS */
